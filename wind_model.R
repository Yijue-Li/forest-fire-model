...
direction <- c(
  NO_WIND = 0,
  NORTHWEST = 1, WEST = 2, SOUTHWEST = 3, NORTH = 4,
  SOUTH = 5, NORTHEAST = 6, EAST = 7, SOUTHEAST = 8
)
# wind strength
strength <- c(
  NO_WIND = 0, LOW = 1, MEDIUM = 3, HIGH = 5
)
# ash use TRUE or FALSE
ash_seq <- c(NO_ASH = 0, ASH = 1)
...
get_neighbors <- function(
    x, y,
    wind_direction = direction["NONE"],
    wind_strength = strength["NONE"],
    ash = FALSE) {
  neighbors <- matrix(nrow = 0, ncol = 2, dimnames = list(NULL, c("x", "y")))
  flag <- 0 # flag to record the direction of current cell
  for (i in -1:1) {
    for (j in -1:1) {
      if (i != 0 || j != 0) {
        flag <- flag + 1
        neighbors <- rbind(neighbors, c(x + i, y + j))
        # find the same direction as wind
        if (flag == wind_direction && wind_strength != 0) {
          for (k in 1:wind_strength) {
            neighbors <- rbind(neighbors, c(x + i, y + j))
          }
          if (ash == 1) {
            b <- wind_strength %/% 2
            neighbors <- rbind(neighbors, c(x + i + b, y + j + b))
          }
        }
      }
    }
...
fire_neighbor <- function(
    old_forest, new_forest, alpha, i, j, nrow, ncol,
    wind_direction, wind_strength, ash) {
  neighbors <- get_neighbors(i, j, wind_direction, wind_strength, ash)
...
# **beta** is the probability of an infected tree being removed
# **fire_prob** is the probability of a tree being infected
# **wind_direction** is the direction of wind
# **wind_strength** is the strength of wind
# **ash** is TRUE or FALSE
#
# return:
# ---
